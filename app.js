...
const documentRows=[...documentList.querySelectorAll('.doc-row')];
documentSearch.addEventListener('input',()=>{const q=documentSearch.value.trim().toLowerCase();let visible=0;documentRows.forEach(row=>{const match=row.dataset.search.includes(q);row.hidden=!match;if(match)visible++});documentCount.textContent=`${visible} ${visible===1?'file':'files'}`});
