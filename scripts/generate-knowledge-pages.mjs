...
for(const document of documents){
  ...
  const localRetrievalLink=document.reference==='SUBSET-098'?'<a class="primary knowledge-download" href="subset-098-rag.html">Open local retrieval prototype</a>':'';
...
<main class="knowledge-page"><div class="eyebrow"><span></span> ERA Set of Specifications 3</div><p class="knowledge-index">Index ${esc(document.index)}</p><h1>${esc(document.reference)}</h1><h2>${esc(document.title)}</h2><dl><div><dt>Approved version</dt><dd>${esc(document.version)}</dd></div><div><dt>Format</dt><dd>${esc(document.type)}</dd></div><div><dt>ERA note</dt><dd>${esc(document.note)}</dd></div><div><dt>Source</dt><dd>European Union Agency for Railways</dd></div></dl><a class="primary knowledge-download" href="${esc(document.url)}">Open authoritative ${esc(document.type)} at ERA <span>↗</span></a>${localRetrievalLink}<p class="source-notice">This catalogue page identifies the official document and links to its authoritative ERA-hosted copy. It does not reproduce the specification.</p></main>
<footer><a class="brand" href="./"><span class="brand-mark">U</span><span>UNISIG <b>Subset</b></span></a><p>Independent catalogue. Not affiliated with UNISIG or ERA.</p><span>© 2026</span></footer></body></html>`);
}
