...
def main() -> None:
    ...
    for number, volume in enumerate(volumes, start=1):
        ...
        page = f'''<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Evidence corpus volume {number}</title><meta name="robots" content="noindex,nofollow,noarchive,nosnippet"><link rel="canonical" href="https://zxj088.github.io/RAG-UNISIG-SUBSET/{filename}"><style>main{{max-width:980px;margin:auto;padding:40px 20px}}article{{border-top:1px solid #ddd;padding:18px 0}}p{{line-height:1.55}}</style></head><body><main><h1>Evidence corpus volume {number} of 4</h1><p>Internal machine-readable extraction. Do not publish without a document-level rights review. Verify engineering decisions against the authoritative ERA source.</p><p><strong>Included documents:</strong> {html.escape(doc_list)}</p>{''.join(volume["blocks"])}</main></body></html>'''
