...
for (const required of ['index.html','404.html','legal.html','subset-098-rag.html','all-evidence-index.json','all-evidence-retrieval.js','rights-policy.json']) assert(fs.existsSync(required), `${required} is missing`);
...
assert(read('app.js').includes('Search evidence'), 'catalogue must link every document to evidence search');
...
assert.equal(rights.default, 'searchable-citation-excerpts');
assert(!read('all-evidence-index.json').includes('OneDrive'), 'public search data must be self-contained and contain no local paths');
